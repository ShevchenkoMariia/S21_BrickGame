	...
	if(!game->pause && isRunning && !exitGame) {
		...
		fullClearWin(&control);
	        for(int i = 1, y = 1; i < HEIGHT - 1; i++, y++) {
				...
				mvwaddch(feild.window, y, x, COLOR_PAIR(game->field[i][j]) | A_INVIS | ' ');
				mvwaddch(feild.window, y, x+1, COLOR_PAIR(game->field[i][j]) | A_INVIS | ' ');
                	}
        	}
		winNextFigure(game);
		winScore(game);
...
void winScore(GameInfo_t* game) {
	...
	mvwprintw(score.window, 1, (score.columns-count-2)/2, " %d ", game->score);
...
void winLevel(GameInfo_t* game) {
	...
	mvwprintw(level.window, 1, (level.columns-count-2)/2, " %d ", game->level);
...
int countDigits(int num) {
    ...
    return (int)log10(abs((double)num)) + 1;
}
...
void winNextFigure(GameInfo_t* game) {
	...
	for(int i = 0, y = 2; i < next.figure.rows; i++, y++) {
		for(int j = 0, x = (nextFigure.columns-next.figure.columns*2)/2; j < next.figure.columns; j++, x+=2) {
			mvwaddch(nextFigure.window, y, x, COLOR_PAIR(game->next[i][j]) | A_BOLD | ' ');
			mvwaddch(nextFigure.window, y, x+1, COLOR_PAIR(game->next[i][j]) | A_BOLD | ' ');
		}
	}
